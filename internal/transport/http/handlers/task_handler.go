...
func (h *TaskHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	response := taskQueueDTO{
		UrgentQueue:  make([]taskDTO, 0),
		RegularQueue: make([]taskDTO, 0),
	}
	for i := range tasks {
		item := newTaskDTO(&tasks[i])
		if item.Queue == taskdomain.QueueUrgent {
			response.UrgentQueue = append(response.UrgentQueue, item)
			continue
		}

		response.RegularQueue = append(response.RegularQueue, item)
	}
